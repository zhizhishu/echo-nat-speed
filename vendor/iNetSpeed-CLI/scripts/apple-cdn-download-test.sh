...
URL="${URL:-https://mensura.cdn-apple.com/api/v1/gm/large}"
MAX="${MAX:-2G}"
TIMEOUT="${TIMEOUT:-10}"
...
msg() {
  ...
  case "$_m" in
    ...
    "iNetSpeed-CLI download test") printf "iNetSpeed-CLI 下载测试" ;;
...
line
info "iNetSpeed-CLI download test"
info "URL: $URL"
if is_zh; then
  info "Limit: $MAX / ${TIMEOUT}s（先到即止）"
else
  info "Limit: $MAX / ${TIMEOUT}s (whichever first)"
fi
...
max_bytes="$(to_bytes "$MAX")"

if [ "$HAS_PV" -eq 1 ]; then
  # 通过第二个 pv 的 numeric 模式持续输出累计字节，避免超时中断时统计为 0
  countfile="$(mktemp)"

  start="$(date +%s.%N 2>/dev/null || date +%s)"

  # 启动管道，用 shell 子进程记录 PID 以便超时杀掉
  sh -c '
    curl '"$HTTP"' -sS -L --output - "'"$URL"'" 2>/dev/null \
      | head -c "'"$max_bytes"'" \
      | pv -s "'"$max_bytes"'" \
      | pv -n -b -i 0.2 >/dev/null 2>"'"$countfile"'"
  ' &
  pipe_pid=$!

  # 超时守护：TIMEOUT 秒后杀掉整个管道
  ( sleep "$TIMEOUT" 2>/dev/null; kill $pipe_pid 2>/dev/null ) &
  timer_pid=$!

  # 等管道结束（正常完成 or 被 timer kill）
  ...
  # 无 pv：curl + head，timeout 用后台 kill 实现
  # 先尝试 Range 方式
  endbyte=$((max_bytes - 1))
  tmpout="$(mktemp)"

  sh -c '
    curl '"$HTTP"' -sS -L -r "0-'"$endbyte"'" -o /dev/null \
      -w "http=%{http_code} time=%{time_total} size=%{size_download} speed=%{speed_download}\n" \
      "'"$URL"'" >"'"$tmpout"'" 2>/dev/null
  ' &
  pipe_pid=$!
  ( sleep "$TIMEOUT" 2>/dev/null; kill $pipe_pid 2>/dev/null ) &
  timer_pid=$!
  ...
  http="$(echo "$out" | awk -F'[ =]' '{for(i=1;i<=NF;i++)if($i=="http")print $(i+1)}')"
  t="$(echo "$out" | awk -F'[ =]' '{for(i=1;i<=NF;i++)if($i=="time")print $(i+1)}')"
  sz="$(echo "$out" | awk -F'[ =]' '{for(i=1;i<=NF;i++)if($i=="size")print $(i+1)}')"
  sp="$(echo "$out" | awk -F'[ =]' '{for(i=1;i<=NF;i++)if($i=="speed")print $(i+1)}')"
